...
def go(args):
    ...
    #Schema of the input and the output
    signature = mlflow.models.infer_signature(X_val[processed_features], y_pred)

    logger.info("Saving model with signature")
    mlflow.sklearn.save_model(
        sk_pipe,
        export_path,
        serialization_format= mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE,
        signature = signature,
        input_example=X_val.iloc[:6]
        )

    # Upload the model we just exported to W&B
    logger.info("Adding and logging artifact")
    artifact = wandb.Artifact(
        args.output_artifact,
        type="model_export",
        description="Trained_random_forest_pipeline_export",
        metadata=rf_config
    )
